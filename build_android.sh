...
FLAGS="-ffunction-sections -funwind-tables -fstack-protector-strong -fPIC -Wall \
	...
	-DANDROID -DPLATFORM_ANDROID -D__ANDROID_API__=${ANDROID_API_VERSION}"
...
for ABI in $ABIS; do
  ...
  make "lib/$ABI/libraylib.a" -j

	CC="$TOOLCHAIN/bin/${CCTYPE}${ANDROID_API_VERSION}-clang"
	...
	$TOOLCHAIN/bin/ld.lld src/*.o -o android/build/lib/$ABI/libmain.so -shared \
		...
		"-L$TOOLCHAIN/sysroot/usr/lib/$LIBPATH/${ANDROID_API_VERSION}" \
...
$BUILD_TOOLS/aapt package -f -m \
	...
	-I "android/sdk/platforms/android-${ANDROID_API_VERSION}/android.jar"

# Compile .java to .class
javac -verbose -source 1.8 -target 1.8 \
  -d android/build/classes \
  -bootclasspath jre/lib/rt.jar \
  -classpath "android/sdk/platforms/android-${ANDROID_API_VERSION}/android.jar" \
  -sourcepath src \
  android/build/src/com/raylib/game/R.java \
  android/build/src/com/raylib/game/NativeLoader.java

# Convert .class to .dex
$BUILD_TOOLS/d8 --classpath "android/sdk/platforms/android-${ANDROID_API_VERSION}/android.jar" --output android/build/dex ./android/build/classes/com/raylib/game/*.class

# Add resources and assets to APK
$BUILD_TOOLS/aapt package -f \
	-M android/build/AndroidManifest.xml -S android/build/res -A assets \
	-I "android/sdk/platforms/android-${ANDROID_API_VERSION}/android.jar" -F game.apk android/build/dex
...
$BUILD_TOOLS/apksigner sign  --ks android/raylib.keystore --out my-app-release.apk --ks-pass pass:raylib game.apk
